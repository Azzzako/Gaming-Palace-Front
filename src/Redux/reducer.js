...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ADD_FAV:
      const addFav = state.allProducts.find(
    ...
    case GET_CART:
      ...
      for (let i = 0; i < action.payload.length; i++) {
        const findProduct = state.allProducts.find(
          (prod) => prod.id === action.payload[i].idproduct
        );
    ...
    case GET_FAVS:
      const prodsInFavs = [];
      for (let i = 0; i < action.payload?.length; i++) {
        const findFavs = state.allProducts.find(
          (prod) => prod.id === action.payload[i].productId
        );
    ...
    case DELETE_FAV:
      const favs =
        ...
        state.favourites.filter((fav) => fav.id !== action.payload.productId);
      return {
        ...state,
        favourites: favs,
      };
