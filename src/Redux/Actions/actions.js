...
export const getArray = (payload) => {
  return async function (dispatch) {
    const response = await axios.get(
      `http://localhost:3001/function2?word=${payload.word}&filter1=${payload.filter1}&filter2=${payload.filter2}&order=${payload.order}`
    );
    return dispatch({
...
export const addFav = (id, item) => {
  return async (dispatch) => {
    const response = await axios.post(
      `http://localhost:3001/favorite/${id}`,
      item
    );
    return dispatch({
      type: ADD_FAV,
...
export const getFavs = (id) => {
  return async (dispatch) => {
    await axios.get(`http://localhost:3001/getfavorites/${id}`).then((res) => {
      dispatch({
        type: GET_FAVS,
...
export const getCart = (id) => {
  try {
    return async (dispatch) => {
      await axios.get(`http://localhost:3001/getcartbyid/${id}`).then((res) => {
        dispatch({
          type: GET_CART,
          payload: res.data,
        });
...
export const totalPayment = (prods) => {
  return async () => {
    await axios
      .post("http://localhost:3001/payment", prods)
      .then(
